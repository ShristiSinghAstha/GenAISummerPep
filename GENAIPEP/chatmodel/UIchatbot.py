...
if prompt:
    ...
    # Append user message to LangChain messages
    ...
    # Retrieve API key (will be set in .env by user)
    api_key = os.getenv("MISTRAL_API_KEY")
    if not api_key:
        st.error("MISTRAL_API_KEY not set. Please add it to your .env file.")
        st.stop()

    # Call Mistral model
    with st.spinner(p["spinner"]):
        model = ChatMistralAI(model="mistral-small-2603", api_key=api_key)
